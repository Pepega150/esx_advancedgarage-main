...
ESX.RegisterServerCallback('esx_advancedgarage:getJobOwnedCars', function(source, cb)
	local ownedCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.ShowVehicleLocation then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE Type = @Type AND `stored` = @stored AND job = @job', { -- job = NULL
			--['@owner'] = xPlayer.identifier,
			['@Type'] = 'car',
			['@stored'] = true,
			['@job'] = xPlayer.job.name,
...
ESX.RegisterServerCallback('esx_advancedgarage:storeJobVehicle', function (source, cb, vehicleProps)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE job = @job AND @plate = plate', {
		...
		if result[1] ~= nil then
			local originalvehprops = json.decode(result[1].vehicle)
			if originalvehprops.model == vehiclemodel then
				MySQL.Async.execute('UPDATE owned_vehicles SET vehicle = @vehicle WHERE job = @job AND plate = @plate', {
					['@job'] = xPlayer.job.name,
					['@vehicle'] = json.encode(vehicleProps),
					['@plate'] = vehicleProps.plate
				}, function (rowsChanged)
